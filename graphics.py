...
class Image(GraphicsObject):
    ...
    def reload (self, win, x, y,  ani):
        self.undraw()
        imagem = Image(Point(x,y), ani)
        imagem.draw(win)
        win.update()
        return imagem

    def reload_walk (self, win, ani, ratio):
    	   fisica = Fisica(win.getWidth(), win.getHeight())
    	   centro = self.anchor
    	   if (ratio>0):
    	   	x = centro.getX()+((self.getWidth())/4.3)
    	   else:
    	     x = centro.getX()-((self.getWidth())/4.3)
    	   y = centro.getY()
    	   if (fisica.limite(self) == "Left"):
    	     x = centro.getX()
    	     ratio = 5
    	   elif (fisica.limite(self) == "Right"):
    	     x = centro.getX()
    	     ratio = -5
    	   imagem = self.reload(win, x+ratio, y, ani)
    	   return imagem

    def reload_attack (self, win, ani): 
            centro = self.anchor
            x = centro.getX()
            y = centro.getY()
            imagem = self.reload(win, x, y, ani)
            return imagem

    def reload_jump (self, win, ani, range_jump, ratio_sleep):
        centro = self.anchor
        x_inicial = centro.getX()
        y_inicial = centro.getY()
        imagem = self.reload(win,x_inicial,y_inicial-3, ani[1])
        i = -2.5
        j = abs(range_jump)
        while (i>range_jump):          
            imagem.move(0,i)
            ...
            win.update()
        centro = imagem.anchor        
        y_auxiliar = centro.getY()
        imagem = imagem.reload(win,x_inicial,y_auxiliar, ani[2])
        while (j>1):
            imagem.move(0,j)        
            time.sleep(ratio_sleep)
            j-=1.5            
            win.update()
        imagem = imagem.reload(win,x_inicial,y_inicial, ani[0])

        return imagem

    def reload_jump_side (self, win, key, ani, range_side, range_jump, ratio_sleep):
        ...
        if (key == "Right"):            
            while (i>range_jump):
                if (fisica.limite(imagem) == "Left" or fisica.limite(imagem) == "Right"):
                    range_side = 0         
                imagem.move(abs(range_side),i)
                cont+=abs(range_side)
                i-=1.5           
                time.sleep(ratio_sleep)
                win.update()
            centro = imagem.anchor        
            y_auxiliar = centro.getY()
            imagem = imagem.reload(win,x_inicial+cont,y_auxiliar, ani[2])
            while (j>1):
                if (fisica.limite(imagem) == "Left" or fisica.limite(imagem) == "Right"):
                    range_side = 0
                imagem.move(abs(range_side),j) 
                cont+=abs(range_side)      
                time.sleep(ratio_sleep)
                j-=1.5            
                win.update()
            imagem = imagem.reload(win,x_inicial+cont,y_inicial, ani[0])

        if (key == "Left"):            
            while (i>range_jump):       
                if (fisica.limite(imagem) == "Left" or fisica.limite(imagem) == "Right"):
                    range_side = 0     
                imagem.move(range_side,i)
                cont+=abs(range_side)
                i-=1.5           
                time.sleep(ratio_sleep)
                win.update()
